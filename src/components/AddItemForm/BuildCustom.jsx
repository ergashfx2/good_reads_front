            ...
            {fields ? fields.map((fieldString, index) => {
                ...
                return (
                    <div key={index} className="mb-3">
                        ...
                        {field.type === 'text' && (
                            <input id={field.label} onChange={handleCustom} type="text" className="form-control" placeholder={`Enter ${field.label}`} />
                        )}
                        {field.type === 'integer' && (
                            <input id={field.label} onChange={handleCustom} type="number" className="form-control" placeholder={`Enter ${field.label}`} />
                        )}
                    </div>
                );
            }) : null}
