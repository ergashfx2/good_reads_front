...
function AdminViewUser({
  ...
  async function handleDeleteSubmit() {
    const params = selected.split("-")(params);
    await apiAdmin
      .post(
        "/delete-items/",
        {
      ...
      {user && collections ? (
        <div>
          ...
          <div className={"row mt-3"}>
            <div
              className={"col-3 d-lg-inline d-sm-none d-md-none d-extra-small"}
            >
              <img
            ...
            <div
              className={"col-4 col-md-12 col-sm-12 d-md-inline d-sm-inline "}
            >
              <h3>{user.name}</h3>
              <p>
                <strong>Bio : </strong>
                {user.bio || "No bio was written yet"}
              </p>
              <p>
        ...
        {items
          ? items.map((item, index) => (
              <Link
                ...
                to={`/edit-item/${item.id}/${item.collection}/${user.id}`}
              >
                <div className={"bg-light"}>
                  <p className={"col-8"} key={index}>
                    <input
                      onChange={handleSelection}
                      id={"items-" + item.id}
                      type={"checkbox"}
