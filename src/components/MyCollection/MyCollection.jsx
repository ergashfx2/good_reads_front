...
function MyCollection() {
    ...
    function handleRename(collection) {
        setDisplay('block')
        const objectWithId = collections.find(obj => obj.id === parseFloat(collection));
    ...
    async function handleRenameSubmit() {
        await api.patch('/update-collection/', {
            col: 'collection_name',
    ...
    async function handleDeleteSubmit (){
        await api.post('/delete-collections/',{collection_id:colId},{
            headers : {
                Authorization : `Bearer ${localStorage.getItem('token')}`
            }
        }).then(res=>{
    ...
    return (
        <div className={"container"}>
            ...
            <section>
                <button onClick={handleCollectionAdd} className={"btn btn-success mx-0 mt-2 mb-3 d-inline"}>Create New
                    Collection
                </button>
            </section>
            <div className="row">
                {collections ? (
                    collections.map((collection) => (
                        <div className={'bg-light row'}>
                            ...
                            <div id={collection.id} onClick={() => handleRename(collection.id)}
                            ...
                            <div id={collection.id} onClick={()=>handleDelete(collection.id)} className={'col-1'}>
                                <Trash3 color={'red'}/>
                            </div>
                ...
                <div
                    ...
                    <div className="text-end">
                        <XSquareFill onClick={() => (setDisplay('none'))} color={'red'} size={30}/>
                    </div>
                    <h5 className={'mt-2'}>Collection Rename</h5>
                    <label className={'form-label'}>Collection name</label>
                    <input value={colname} onChange={(e) => setColname(e.target.value)} className={'form-control'}/>
                    <button onClick={handleRenameSubmit} className={'btn btn-warning mt-3'}>Rename</button>
                </div>
