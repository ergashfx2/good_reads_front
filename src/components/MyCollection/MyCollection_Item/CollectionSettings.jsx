...
const CollectionSettings = () => {
  ...
  return (
    <div className={"container"}>
      <h3
        style={{ height: "4rem" }}
        className={"text-warning pt-2 bg-primary rounded"}
      >
        Collection creation
      </h3>
      <Form className={"form-control"}>
        <Form.Group className="mb-3">
          <Form.Label className={"fw-bold"}>
            Enter title of collections{" "}
          </Form.Label>
          <Form.Control onChange={handleTitle} type="text" />
        </Form.Group>
        <div className={"alert alert-info"}>
          These are fixed collection fields please do not change anything and
      ...
      />
      <UploadImages />
      <Button className={"w-100"} onClick={handleSubmit} variant={"primary"}>
        Create Now
      </Button>
    </div>
  );
